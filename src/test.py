...
def main():
    ...
    while idx >= 0:
        ...
        if 0 <= idx <= train_set.data.size()[0]:
            ...
            imgnoise = img + torch.rand(img.shape)

            # plt.imshow(img, cmap='gray')
            # plt.show()

            imgnoise = imgnoise.to(device=device)
            # print('break 7: ', torch.max(img), torch.min(img), torch.mean(img))
            #print('break 8 : ', img.shape, img.dtype)
            imgnoise = imgnoise.view(1, imgnoise.shape[0]*imgnoise.shape[1]).type(torch.FloatTensor)
            #print('break 9 : ', img.shape, img.dtype)
            with torch.no_grad():
                output = model(imgnoise)
            ...
            imgnoise = imgnoise.view(28, 28).type(torch.FloatTensor)
